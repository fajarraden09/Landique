<?php
  ...
  $query = "SELECT * FROM `tb_rental` INNER JOIN `tb_pelanggan` ON `tb_rental`.`pelangganid` = `tb_pelanggan`.`pelangganid` INNER JOIN `tb_users` ON `tb_users`.`userid` = `tb_rental`.`userid` INNER JOIN `tb_jenis` ON `tb_jenis`.`kd_jenis` = `tb_rental`.`kd_jenis` WHERE `tb_rental`.`id_rental` = '$id_rental'";
          ...
          <div class="page-title-box">
              <div class="btn-group float-right">
                  <ol class="breadcrumb hide-phone p-0 m-0">
                      ...
                      <li class="breadcrumb-item active">Detail Transaksi rental</li>
                  </ol>
              </div>
              <h4 class="page-title">Detail Transaksi rental</h4>
          </div>
      </div>
  </div>
      ...
      <div class="col-12">
        <div class="card m-b-30">
          ...
          <div class="table-responsive">
            ...
            <a href="page/cetak_transaksi.php?id=<?= $row['id_rental']; ?>" class="btn btn-primary" target="_blank">Cetak Invoice</a>
          </div>
        </div>
      </div>
      <!-- end col -->
