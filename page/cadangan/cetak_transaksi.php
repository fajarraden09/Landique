<?php
  ...
  $query = "SELECT * FROM `tb_rental` INNER JOIN `tb_pelanggan` ON `tb_rental`.`pelangganid` = `tb_pelanggan`.`pelangganid` 
  INNER JOIN `tb_users` ON `tb_users`.`userid` = `tb_rental`.`userid` INNER JOIN `tb_jenis` ON `tb_jenis`.`kd_jenis` = `tb_rental`.`kd_jenis` 
  WHERE `tb_rental`.`id_rental` = '$id_rental'";
...
<table>
            ...
            <tr>
              <th align="left">Status Pengambilan PS</th>
              <td>:</td>
...
<table width='100%' cellpadding='5' cellspacing='0' border="1">
    ...
    <tbody>
      <tr>
        ...
        <th>Rp. <?= number_format($row['tarif']); ?></th>
      </tr>
    </tbody>
  </table>
