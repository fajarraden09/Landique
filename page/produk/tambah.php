...
if (isset($_POST['tambah'])) {
  ...
  if ($row['produk'] !== $produk) {
    ...
    // jika produk ada yg sama
    if ($result_produk > 0) {
      $pesan_error_user = "produk produk <b>$produk</b> sudah digunakan <br>";
    }
  }

  // jika tidak ada error
  if ($pesan_error_user == "" && $pesan_error_pass == "") {
    $query = mysqli_query($conn, "INSERT INTO `tb_produk`(`id_produk`, `produk`, `kategori`, `harga`, `harga_sewa`, `deskripsi`, `stok`) VALUES ('$id_produk', '$produk', '$kategori', '$harga', '$harga_sewa', '$deskripsi', '$stok')");
    if ($query) {
      echo "
      <script>
        alert('Data Produk baru $produk berhasil ditambahkan');
